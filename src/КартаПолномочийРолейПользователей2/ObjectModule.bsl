...
Функция СформироватьМакетТаблицыОтчета(пМассивИменРолей, НеВыводитьОбъектБезПрав) Экспорт
	...
	ОбластьРоли     = Макет.ПолучитьОбласть("Роли");
	ОблЗаголовок    = Макет.ПолучитьОбласть("Заголовок");
	ОблШапкаНачало  = Макет.ПолучитьОбласть("ШапкаТаблицы|Начало");
	ОблШапкаПраво   = Макет.ПолучитьОбласть("ШапкаТаблицы|Право");
	ОблСтрокаНачало = Макет.ПолучитьОбласть("СтрокаТаблицы|Начало");
	ОблСтрокаПраво  = Макет.ПолучитьОбласть("СтрокаТаблицы|Право");
	ОблПодвалНачало = Макет.ПолучитьОбласть("ПодвалТаблицы|Начало");
	ОблПодвалПраво  = Макет.ПолучитьОбласть("ПодвалТаблицы|Право");
	ОблСтрокаНачалоБезПрав = Макет.ПолучитьОбласть("СтрокаТаблицыБезПрав|Начало");
	ОблСтрокаПравоБезПрав  = Макет.ПолучитьОбласть("СтрокаТаблицыБезПрав|Право");
	...
	Для Каждого МетаРоль Из МассивРолей Цикл
		СтрРоли = СтрРоли + ", " + МетаРоль.Синоним;
	КонецЦикла;
	
	ОблЗаголовок.Параметры.ВидМетаданных = "Роли";
	ТабДок.Вывести(ОблЗаголовок);
	ТабДок.НачатьГруппуСтрок(,Ложь);
	ОбластьРоли.Параметры.Роли = Сред(СтрРоли,3);
	ТабДок.Вывести(ОбластьРоли);
	ТабДок.ЗакончитьГруппуСтрок();
	
	// вывод прав на конфигурацию
	ОблЗаголовок.Параметры.ВидМетаданных = "Права на конфигурацию";
	ТабДок.Вывести(ОблЗаголовок);
	ТабДок.НачатьГруппуСтрок(,Ложь);
	...
	ВывестиШапкуПоТипуМетаданных(ШапкуВывелиВОтчет, ТабДок, ОблШапкаНачало, ОблШапкаПраво, ОписаниеТипаМетаданных, ТаблицаВидовДоступа);
	
	ОблСтрокаНачало.Параметры.ТипОбъекта = "Права на конфигурацию";
	...
	Для Каждого СтрокаПраво Из ТаблицаВидовДоступа Цикл
		...
		Для Каждого МетаРоль Из МассивРолей Цикл
			...
			КонецЕсли;
		КонецЦикла;
		ОблСтрокаПраво.Параметры.Право = ЕстьПраво;
		ТабДок.Присоединить(ОблСтрокаПраво);
	КонецЦикла;
	
	ВывестиПодвалПоТипуМетаданных(ШапкуВывелиВОтчет, ТабДок, ОблПодвалНачало, ОблПодвалПраво, ТаблицаВидовДоступа);
	ТабДок.ЗакончитьГруппуСтрок();
	...
	//ОблЗаголовок.Параметры.ВидМетаданных = "Общие объекты конфигурации";
	//ТабДок.Вывести(ОблЗаголовок);
	//ТабДок.НачатьГруппуСтрок(,Ложь);
	...
	Для Каждого ОписаниеТипаМетаданных Из ТаблицаОбъектовМетаданных Цикл
		...
		ОблЗаголовок.Параметры.ВидМетаданных = ОписаниеТипаМетаданных.Представление;
		ТабДок.Вывести(ОблЗаголовок);
		...
		Для Каждого ЭлементСписка Из СписокОбъектов Цикл
			...
			ОбластьНачало = ?(ЕстьХотяБыОдноПраво, ОблСтрокаНачало, ОблСтрокаНачалоБезПрав);
			ОбластьПраво = ?(ЕстьХотяБыОдноПраво, ОблСтрокаПраво, ОблСтрокаПравоБезПрав);
			ОбластьНачало.Параметры.ТипОбъекта = ЭлементСписка.Представление;
			
			ВывестиШапкуПоТипуМетаданных(ШапкуВывелиВОтчет, ТабДок, ОблШапкаНачало, ОблШапкаПраво, ОписаниеТипаМетаданных, МассивСтрок); //шапка
		...
		ВывестиПодвалПоТипуМетаданных(ШапкуВывелиВОтчет, ТабДок, ОблПодвалНачало, ОблПодвалПраво, МассивСтрок); //подвал
		ТабДок.ЗакончитьГруппуСтрок();
