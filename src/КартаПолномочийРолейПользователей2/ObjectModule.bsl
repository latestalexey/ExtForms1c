...
Функция ПрочитатьОтчетИзФайлаXML(СтруктураНастроек) Экспорт
	...
	Пока ФайлXML.Прочитать() Цикл
		...
		Если ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ФайлXML.Имя = "КартаПолномочийРолейПользователей" Тогда
			...
			Пока ФайлXML.Прочитать() Цикл
				...
				// права на конфигурацию
				Если ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ФайлXML.Имя = "Конфигурация" Тогда
					ТаблицаПрав = ПрочитатьПроизвольноеЗначениеИзXML(ФайлXML, "Конфигурация");
				КонецЕсли;
	...
	// вывод отчета:
	// вывод заголовка и заполнение настроек
	// вывод таблиц прав
	// завершение
